...
## Graph Observations
A [Check](../../api-reference/check-service#check) request of the form `Check(namespace:object#relation@subject)` will be evaluated by searching the graph starting at an object and going through it's relations in search for the matching subject. A Check request is allowed if there is such a path.
